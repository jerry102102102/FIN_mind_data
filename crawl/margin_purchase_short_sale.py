...
def fetch_margin_purchase_short_sale(start_date, end_date, token, stock_id):
    ...
    if "Requests reach the upper limit" in data['msg']:
        print(f"Requests reach the upper limit for date {start_date}. Stopping execution.")
        return None
    if data['msg'] == 'success':
        if len(data['data']):
...
def main(stock_list, start_date_str, end_date_str, last_time_execute_stock):
    ...
    for idx in range(last_time_execute_stock, len(stock_list)):
        ...
        if not data.empty:
            ...
            filename = os.path.join('data', "margin_purchase_short_sale.csv")
